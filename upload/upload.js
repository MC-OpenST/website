        ...
        <div
          ...
          <div class="p-8">
            ...
            <div v-if="userToken && step === 1"
              ...
              <p class="text-[13px] text-gray-500 font-bold uppercase tracking-[0.2em]">分类标签</p>
              
                <div class="space-y-3">
                  <div class="bg-white/[0.03] p-6 rounded-[2rem]">
                    <div v-for="item in processedConfig" :key="item.mainCat" class="border-l-2 border-[#40B5AD]/20 pl-4 pt-5 mb-8 last:mb-0">
                      
                      <!-- 大类标题 -->
                      <h3 class="text-sm font-medium text-[#40B5AD] mb-6 flex items-center gap-2">
                        <span class="w-1.5 h-1.5 rounded-full bg-[#40B5AD]"></span>
                        {{ item.mainCat }}
                      </h3>
                
                      <!-- A. 渲染所有“扁平”标签 (包含原始一级和退化后的二级) -->
                      <div v-if="item.flattened.length" class="flex flex-wrap gap-2 mb-6">
                        <button v-for="tag in item.flattened" :key="tag" @click="toggleTag(tag)"
                                :class="form.tags.includes(tag) ? 'bg-[#40B5AD] text-black border-[#40B5AD]' : 'bg-white/5 text-white/40 border-white/10 hover:border-white/30'"
                                class="px-3 py-1.5 rounded-lg text-[11px] transition-all border">
                          {{ tag }}
                        </button>
                      </div>
                
                      <!-- B. 渲染所有“嵌套”标签 (真正有子级的二级) -->
                      <div v-for="sub in item.nested" :key="sub.subCat" class="mb-8 last:mb-0">
                        <!-- 二级下划线标题 -->
                        <div class="flex mb-4">
                          <button @click="toggleTag(sub.subCat)"
                                  :class="form.tags.includes(sub.subCat) ? 'text-[#40B5AD] border-[#40B5AD]' : 'text-white/40 border-white/10 hover:text-white/60'"
                                  class="px-0 py-0.5 border-b-2 text-[12px] transition-all tracking-wider">
                            {{ sub.subCat }}
                          </button>
                        </div>
                        <!-- 三级按钮组 -->
                        <div class="flex flex-wrap gap-2">
                          <button v-for="tag in sub.tags" :key="tag" @click="toggleTag(tag, sub.subCat)"
                                  :class="form.tags.includes(tag) ? 'bg-[#40B5AD] text-black border-[#40B5AD]' : 'bg-white/5 text-white/40 border-white/10 hover:border-white/30'"
                                  class="px-3 py-1.5 rounded-lg text-[11px] transition-all border">
                            {{ tag }}
                          </button>
