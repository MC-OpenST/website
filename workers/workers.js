...
export default {
    async fetch(request, env) {
        ...
        try {
            // --- [1] OAuth 令牌交换 ---
            if (url.pathname === '/workers/exchange-token') {
            ...
            // --- [2] 权限校验 ---
            if (url.pathname === '/workers/check-admin') {
            ...
            // --- [3] 管理员修改数据 ---
            if (url.pathname === '/workers/admin/update-info' && request.method === 'POST') {
            ...
            // --- [6] Wiki 专用：关键逻辑更新 ---
            if (url.pathname === '/workers/wiki/submit-archive' && request.method === 'POST') {
                const fd = await request.formData();
